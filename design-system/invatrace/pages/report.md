...
## Steps
1. **Location.** Auto-request geolocation on mount; show "Locating…", "Denied", "Unavailable", or a success card with accuracy in metres. Continue only when a camera-time or refreshed GPS fix is accurate within 100 metres; manual coordinates are not accepted as automated field evidence.
...
4. **Preview.** Photo, then metadata card: Species · Outcome · Location with GPS accuracy · Extent · Notes. Model version bar. Big "Submit report" primary button (with Send icon).
